...
public class GameManager : MonoBehaviour
{
    ...
    [Header("Player saat GameOver")]
    public GameObject playerFallImagePrefab; // prefab berisi sprite jatuh
    private GameObject fallingPlayerInstance;
    public GameObject playerIdleImagePrefab; // Drag prefab gambar player idle
    private GameObject idlePlayerInstance;
    ...
    public bool IsPreDoubleMode()
    {
        return currentTime <= 40f && currentTime > 30f;
    }
    ...
    public void EndByTime()
{
    if (!isGameOver)
    {
        ...
        // Nonaktifkan player asli
        player.gameObject.SetActive(false);

        // Spawn gambar player idle jika prefab disiapkan
        if (playerIdleImagePrefab != null)
        {
            Vector3 spawnPos = player.transform.position;
            idlePlayerInstance = Instantiate(playerIdleImagePrefab, spawnPos, Quaternion.identity);
        }
    ...
    //     // Hapus gambar jatuh jika perlu
    //     // if (fallingPlayerInstance != null)
    //     // {
    //     //     Destroy(fallingPlayerInstance, 1f);
    //     // }
    // }
    ...
    public void GameOver(bool fromObstacle = true)
    {
        ...
        {
            isGameOver = true;
            player.gameObject.SetActive(false); // non-aktifkan player utama

            // tampilkan gambar jatuh di posisi player
            if (playerFallImagePrefab != null)
            {
                Vector3 spawnPos = player.transform.position;
                fallingPlayerInstance = Instantiate(playerFallImagePrefab, spawnPos, Quaternion.identity);
            }

            // Tampilkan UI Game Over setelah delay
            // StartCoroutine(ShowGameOverUIAfterDelay(1.2f)); // waktu muncul panel

            // cek game over panel & tampilkan
    ...
    public bool IsDoubleModeActive()
    {
        return isDoubleScoreAndSpeedActive;
    }
    ...
    public void RestartGame()
    {
        ...
        // Restart player idle
        // Hapus instansi player idle jika ada
        if (idlePlayerInstance != null)
        {
            Destroy(idlePlayerInstance);
        }

        // Restart player fall
        if (fallingPlayerInstance != null)
        {
            Destroy(fallingPlayerInstance);
        }
